...
self.onmessage = (e) => {
    ...
    let startIndex = idToIndex(startId);
    let endIndex = idToIndex(endId);
    let currentIndex;
    let nextIndex;
    ...
    for (let k=0;k<node_li.length;k++) {    // Intermediate node. k = index of node, not id
        updateNode(k, "#FFA849");   // Sets node as being processed
        for (let i=0;i<node_li.length;i++) {    // Start node. i = index of node, not id
            updateNode(i, "#FFA849");   // Sets node as being processed
            for (let j=0;j<node_li.length;j++) {    // End node. j = index of node, not id
